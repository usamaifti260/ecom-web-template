export async function fetchClientProducts(schemaSlug = '') {
 ...
 /* if (process.env.NODE_ENV === 'development') {
    console.log('Using static clothing data for development');
    try {
      // Import the JSON data
      const products = await import('../data/products.json');
      const clothingData = products.default;
      
      // Transform the data to ensure all required properties are present
      const transformedProducts = clothingData.map(product => ({
        id: product.id,
        name: product.name,
        ...
        brand: product.brand || 'Hathkari Official',
      ...
      console.log(`Loaded ${transformedProducts.length} clothing products from local JSON`);
      return transformedProducts;
    } catch (error) {
      console.error('Error loading local clothing products data:', error);
  ...
  const dashboardApiUrl = process.env.DASHBOARD_API_URL || 'https://web-portal-backend-production.up.railway.app/api/data/website/68750122b89a602f3ddbfe3d';
...
export async function fetchClientCategories(schemaSlug = 'hathkaricategoriesschema') {
  // In development, use static mock data from JSON file
 /* if (process.env.NODE_ENV === 'development') {
